...
int main() {
  ...
  while (true) {
      ...
      getline(myfile, line);
      if (!cin.good()) throw std::runtime_error("input stream was bad"); // if input stream isn't good throw error
      if (not myfile) {
  ...
  while(message.length() > 0) {
      ...
      if (!cin.good()) throw std::runtime_error("input stream was bad");
      getline(cin, message);
      if (!cin.good()) throw std::runtime_error("input stream was bad");
